...
- 힌트를 참고해서 풀었다... (Linked list)

- up,down은 문제가 되지 않는다. 단순하게 prev, next를 옮겨가면된다
- 다만 지우는것과 복구에서는 조금 복잡했다
  - 일단 **<u>둘 다 prev node랑 next node가 Not None인 경우</u>**에만 연산을 진행한다.
  - 지우는 명령어
    - prev node의 next를 현재 선택된 노드의 next로 연결한다.
    - next node의 prev를 현재 선택된 노드의 prev로 연결한다.
    - 현재 노드를 "X" 처리하고, stack에 추가한다.
  - 복구 명령어는 지우는 명령어의 딱 반대로 진행한다.

#### 근데 Class로 푸는게 더 효과적이었을 것 같다.
